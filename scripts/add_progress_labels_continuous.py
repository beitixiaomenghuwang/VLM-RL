#!/usr/bin/env python3
"""
为 LeRobot 数据集添加连续任务进度标签（0-100%）。

用法：
    conda activate lerobot
    python scripts/add_progress_labels_continuous.py \
...
def add_progress_labels_continuous(input_path: str, output_path: str, overwrite: bool = False):
    """
    为数据集的每一帧添加连续进度标签（0-100%）。
    ...
    logger.info(f"✅ 成功创建带连续进度标签的数据集: {output_path}")
    ...
    if first_file and first_file.exists():
        ...
        if 'observation.progress' in table.column_names:
            ...
            for i in range(min(3, len(progress_col))):
                ...
                logger.info(f"  帧 {frame_idx}: progress = {progress_val:.4f} ({progress_val*100:.1f}%)")
...
def main():
    parser = argparse.ArgumentParser(description="为 LeRobot 数据集添加连续任务进度标签（0-100%）")
    parser.add_argument(
        "--input_dataset",
        type=str,
        required=True,
    ...
    add_progress_labels_continuous(
